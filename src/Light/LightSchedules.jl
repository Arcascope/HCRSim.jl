...
function make_pulse(t, tstart, tend)

	steep=30.0
	return(0.5*tanh(steep*(t-tstart))-0.5*tanh(steep*(t-tend)));

end


"""
Helper function which will generate a regular light schedule repeating forever. 

Starts off with darkness.....

"""
function LightRegularCpp(t; Intensity=150.0, PhotoPeriod=16.0, period=24.0)

	s=t % period
	return(Intensity*make_pulse(s,0.0,PhotoPeriod))
end
...
"""
	...
	RegularLight(t; Intensity=150.0, PP=16.0, period=24.0)
"""
function RegularLight(t; Intensity=150.0, PhotoPeriod=16.0, period=24.0)
